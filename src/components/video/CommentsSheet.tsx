...
export function CommentsSheet({ videoId, isOpen, onClose, onCommentAdded }: CommentsSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      {/* Sheet */}
      <div className="relative w-full max-w-lg bg-[#1A1F2E] rounded-t-3xl max-h-[70vh] flex flex-col animate-slide-up">
        {/* Handle */}
            ...
            <>
              {comments.map((comment) => (
                <div key={comment.id} className="flex gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#6366F1] to-[#14B8A6] flex items-center justify-center text-white font-medium text-sm flex-shrink-0">
              ...
              {hasMore && (
                <button
                  ...
                  className="w-full py-2 text-[#6366F1] text-sm font-medium"
                >
                  Load more comments
                </button>
              )}
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-white/10">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-white/10 text-white placeholder-white/40 rounded-full px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#6366F1]"
            />
            <button
              type="submit"
              disabled={!newComment.trim() || isSubmitting}
              className="w-10 h-10 rounded-full bg-gradient-to-r from-[#6366F1] to-[#14B8A6] flex items-center justify-center text-white disabled:opacity-50"
            >
              {isSubmitting ? (
