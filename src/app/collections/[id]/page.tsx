      ...
      <div className="bg-[#1A1F2E] rounded-2xl w-full max-w-md mx-4 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-[#0A0E1A] text-white px-4 py-3 rounded-xl outline-none" autoFocus />
          </div>
          <div>
            <label className="block text-white/50 text-sm mb-2">Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full bg-[#0A0E1A] text-white px-4 py-3 rounded-xl outline-none resize-none" />
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            <div className={`w-5 h-5 rounded-md flex items-center justify-center ${isPrivate ? 'bg-[#6366F1]' : 'bg-[#0A0E1A] border border-white/20'}`} onClick={() => setIsPrivate(!isPrivate)}>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" disabled={!name.trim() || isSubmitting} className="flex-1 py-3 rounded-xl bg-[#6366F1] text-white font-medium disabled:opacity-50">{isSubmitting ? 'Saving...' : 'Save'}</button>
          </div>
        </form>
      </div>
    </div>
...
export default function CollectionPage() {
  ...
  if (!c.isAuthenticated) {
    return <div className="min-h-screen bg-[#0A0E1A] flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#6366F1]" /></div>;
  }
      ...
      <main className="flex-1 md:ml-64 pb-20 md:pb-0">
        ...
        {c.collection && (
          <div className="p-4 border-b border-white/5">
            ...
            {c.videos.length > 0 && (
              <Link href={`/feed?playlist=${c.collection.id}`} className="mt-4 w-full flex items-center justify-center gap-2 py-3 bg-[#6366F1] text-white font-medium rounded-xl hover:opacity-90 transition-opacity">
        ...
        <div className="p-4">
          {c.isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <Link href="/feed" className="inline-block mt-4 px-6 py-2 bg-[#6366F1] text-white rounded-full">Browse videos</Link>
            </div>
          ) : (
            <div className="space-y-3">
              ...
              {c.hasMore && <button onClick={c.loadMoreVideos} className="w-full py-3 text-[#6366F1] hover:text-[#5558E3] text-sm font-medium">Load more</button>}
            </div>
          )}
        </div>
      </main>
